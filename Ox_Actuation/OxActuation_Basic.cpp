// Ox Actuation Basic algorithm code, Hayden Lau, Mar 2014
// UTAT Rocketry
// basic algorithm:
// move to spot
// on spot, brake
// while not stopped, check if stopped
// to do: make this a library, encapsulate lastPosCount as just 'pos' or something
// make getPos() and setPos() commands
...
namespace OxActuation
{
	...
	uint16_t updatePos(); // updates lastPosCount, returns counted increment
...
namespace OxActuation
{
	...
	void moveTo(uint16_t target)
	{
		...
		if (target > OX_X_MAX || target < OX_X_MIN)
			return;
	...
	uint16_t updatePos()
	{
		...
		// reset counter
		TCNT1 = 0;
		// add counter to lastPosCount (accounting for direction of movement)
		if (lastVelocity > 0)
		else
		// wraparound
		lastPosCount = wrapPos(lastPosCount);
		return 
	}
